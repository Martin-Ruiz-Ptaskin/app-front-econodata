...
export class FoundListServiceService {
  private apiUrl = environment.apiUrl+"fondos/"; // Usa la URL del environment
  constructor(private http: HttpClient,private error:ErrorHttpsService ) { }
  currentPage:number=0
  getData(): Observable<any> {
    let url = this.apiUrl + "getFounds.php";
...
getNextPage(page:number){
  let url = this.apiUrl + "getFounds.php"+"?page="+page;
    return this.http.get<any>(url).pipe(
      map(response => {
        // Aquí validamos si el código de estado no es 200
        if (response.status !== 200) {
...
getFoundByName(name:any){
  console.log(encodeURIComponent(name))
  let url = this.apiUrl + "getFoundByName.php"+"?name="+encodeURIComponent(name);
    return this.http.get<any>(url).pipe(
      map(response => {
        // Aquí validamos si el código de estado no es 200
        if (response.status !== 200) {
